---
name: install-argo-mcp-claude
description: Walk the user through connecting the hosted Argo MCP server (https://mcp.argo.games/mcp) to Claude Code or Claude.ai so the Argo tools become available. Use when the user says "connect Argo to Claude", "install Argo MCP in Claude", "set up Argo in Claude Code", "set up Argo in Claude.ai", "hook up my campaign", or otherwise asks how to plug the Argo platform into a Claude surface. For Codex or ChatGPT setup, use `install-argo-mcp-codex` instead. Do not use for unrelated MCP setup or for installing other plugins.
---

# Install the Argo MCP (Claude)

You are helping a Game Master connect the hosted Argo MCP server to a **Claude** client (Claude Code or Claude.ai). The MCP exposes the GM's campaign data (mnemons, sessions, forum, guilds, etc.) as tools the assistant can call.
...
It uses OAuth2 with Dynamic Client Registration. No tokens or secrets need to be pasted by hand — Claude handles auth in a browser on first connect.

> **Not on Claude?** If the GM is using Codex or ChatGPT, switch to the `install-argo-mcp-codex` skill instead.

## Decide the surface

Ask the GM which Claude surface they're using:
